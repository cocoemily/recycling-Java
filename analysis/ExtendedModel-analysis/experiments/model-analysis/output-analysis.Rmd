...
Analysis of cumulative outputs for entire landscape at different points in model run.
...
endoutputs = enddata[,cum.outputs]
rm(enddata)
M = cor(endoutputs)
...
midoutputs = middata[,cum.outputs]
rm(middata)
...
for(i in unique(two.tech$model_year)){
  ...
  yearoutputs = yeardata[,cum.outputs]
...
plot1 = ggplot(plot.cor.time, aes(x = model_year, y = correlation, color = signf.val)) +
  geom_point(size = 0.1) +
  #geom_smooth(se = F) +
...
plot2 = ggplot(plot.cor.time, aes(x = model_year, y = correlation, color = signf.val)) +
  geom_point(size = 0.1) +
  #geom_smooth(se = F) +
  facet_grid(output1 ~ output2) +
  scale_x_reverse(breaks = c(500000, midyear, endyear)) +
...
#### Comparison of two technologies and multiple technologies
```{r cor-compare, include = T, warning=F}
