...
foreach (d=1:length(dirs)) %dopar% { 
  ...
  end_data = data[which(data$model_year == 200000), c(parameters, "model_year", "row", "col", "recycling.intensity", "flake.count", "nodule.count")]
  end_data = end_data[1:5000,]
  mid_data = data[which(data$model_year == 350000), c(parameters, "model_year", "row", "col", "recycling.intensity", "flake.count", "nodule.count")]
  ...
  readr::write_csv(ad, paste0("/scratch/ec3307/recycling-Java/output/layer-output/", filename, "_gridded-RI.csv"), num_threads=1)
}
