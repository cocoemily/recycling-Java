...
foreach (d=1:length(dirs)) %dopar% { 
  ...
  for(run in unique(data$run)) {
    ...
    avg = mean(rundata$cortex.ratio, na.rm = T)
    med = median(rundata$cortex.ratio, na.rm = T)
    
    n = length(rundata$cortex.ratio)
    error = qt(0.975,df=n-1) * sd(rundata$cortex.ratio, na.rm = T)/sqrt(n)
    lower = avg - error
    upper = avg + error
  ...
  write_csv(results, file = paste0("/scratch/ec3307/updated-recycling-Java/recycling-Java/output/layer-output/", filename, "_CR-CI-results.csv"), num_threads=1)
