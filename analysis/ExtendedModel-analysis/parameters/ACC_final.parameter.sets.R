...
options(scipen=10)
parameters = data.frame(modelType = character(),
                        of = character(),
                        name = character(),
                        size = numeric(),
                        startYear = numeric(),
...
mu.values = c(2) #mu is held constant
...
save(fparameters, file = "analysis/ExtendedModel-analysis/parameters/accumulation-modelrun-params.RData")

setwd("..") #moves up a directory
write.csv(fparameters, file = "run-scripts/ExtendedModel-model-runs/accumulation_parameters.csv", row.names=F)
...
for(i in c(1:2)) {
  ...
  endrow = i * 935
  filename = paste0("run-scripts/ExtendedModel-model-runs/accumulation/parameters", i, ".csv")
