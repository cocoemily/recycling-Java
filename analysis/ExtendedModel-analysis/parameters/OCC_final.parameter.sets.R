...
save(fparameters, file = "ExtendedModel-analysis/parameters/occupation-modelrun-params.RData")

setwd("..") #moves up a directory
write.csv(fparameters, file = "run-scripts/ExtendedModel-model-runs/occupation_parameters.csv", row.names=F)
...
for(i in c(1:6)) {
  ...
  endrow = i * 935
  filename = paste0("run-scripts/ExtendedModel-model-runs/occupation/parameters", i, ".csv")
